...
async function seedTags() {
  ...
  const tags = await Promise.all(Array.from({ length: TAGS.length }).map((_, index) => prisma.tag.create({ data: { name: TAGS[index] } })))
  ...
  const seedOne = async () =>
    prisma.wishlistEntry.create({
      data:
      {
        ...
        link: faker.internet.url(), linkText: faker.lorem.words({ min: 1, max: 3 }),
        price: faker.number.float({ min: 0.01, max: 1_000, fractionDigits: 2 }),
        comments: faker.lorem.sentence(),
        status: WISHLIST_STATUS[faker.number.int({ min: 0, max: WISHLIST_STATUS.length - 1 })]
      }
    })

  console.time("Seeding wishlist entries")
  const wishlistLength = faker.number.int({ min: 5, max: 15 })
  const wishlistEntries = await Promise.all(Array.from({ length: wishlistLength }).map(async () => seedOne()))
...
async function seed() {
  ...
  const [wishlistEntries, tags] = await Promise.all([seedWishlistEntries(), seedTags()])
  ...
  wishlistEntries.forEach(async (entry) => {
    ...
    for (let i = 0; i <= tagsLength; i++) {
      ...
      await prisma.wishlistEntryTag.create(
        { data: { wishlistEntryId: entry.id, tagId: localTags.splice(faker.number.int({ min: 0, max: localTags.length - 1 }), 1)[0].id } })
    }
  })
}
